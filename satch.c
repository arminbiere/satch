...
// except for 'satch_{compile,identifier,version}'. These functions return
// build information and are implemented in 'config.c', which in turn is
// automatically generated by 'mkconfig.sh'.  If you do not need those, nor
// the internal proof checking code in 'catch.[ch]', then you can just link
// against this file.
//
// In order to to disable proof checking and debugging (and avoid a
// link-time dependency on 'catch.o') you should compile with 'NDEBUG'
// defined.  Not defining 'NDEBUG' enables of course also assertion checking
// in general, but also witness checking and includes logging code.  The
// latter still needs to be enabled at run-time through
// 'satch_enable_logging_messages' though.
//
...
// do sanity checking on the selected feature macros we still get the
...
#ifdef NRESTART
#ifdef NSTABLE
#error "'NRESTART' implies 'NSTABLE' (latter should not be defined then)"
#else
#define NSTABLE
#endif
#endif
...
// Watches are made of a header and a clause.  For binary clauses the
// blocking literal and thus the header is enough and we can omit the
// clause pointer.
//
...
#ifdef NSTABLE
  struct averages averages;	// focused exponential moving averages
#else
...
static void
enqueue (struct satch *solver, unsigned idx, unsigned except)
{
  ...
  if ((link->stamp = ++queue->stamp))	// Check for overflow.
    {
      ...
      if (idx != except && !solver->values[lit])
	queue->search = idx;
    }
  else
  ...
  while (!conflict && p != end_watches)
	...
	// enabled or both cases (binary and non-binary clauses) if they are.
	{
	  ...
	  if (replacement_value > 0)	// replacement literal true thus
	    {
#ifndef NBLOCK
	      q[-2].header.blocking = replacement;	// update blocked literal
#endif
	    }
