...
struct clause
{
  ...
  // From 'solver->statistics.added' needed for stable sorting and logging.
  uint64_t id;
#endif
...
// focused mode though).  This could in principle be figured out at compile
// time but leads to extremely complex preprocessor macro checking code.
// Instead we initialize the queue on-demand if 'size' is not big enough.
...
// currently the API does not allow yet to add further clause after the
// first call to 'satch_solve'.
...
propagate_literal (struct satch *solver, unsigned lit)
{
  ...
  // and clause memory.  And assignment is also counted as one cache line
  // access ('tick') but we do not take reading assigned values into
  // account, assuming those single byte accesses are shared.
...
// Backtrack to a certain decision level. This in essence just unassigns the
// literals assigned on higher decision level. Resets the decision level and
// the propagation pointer.

// It is slightly more complicated though since we need first to push back
// unassigned variables back to the binary heap for (E)VSIDS and second
// update the search index for the VMTF queue. Third without clause learning
// ('NLEARN' defined) learned clauses are only used for computing scores and
// back-jumping and during backtracking have to be deleted.
...
// Without using radix sort the time spent in 'sort_analyzed'
// can reach 20% of the total running time (including both time spent in
// focused and stable mode) even though 'sort_analyzed' is by default only
// used in focused mode for the VMTF decision heuristic.  This might then
// increase relative time spent in focused mode since we do not want to
// account for sorting with our 'ticks' counter.  We have seen cases where
// this effect lead to 70% time spent in focused mode and only 30% time in
// stable mode.  Using faster radix sorting allows to achieve a more
// balanced split of search time into focused and stable mode.
...
// implement quick sort at this point.  Note also that even quick-sort
...
// *INDENT-ON*
  ...
  if ((solver->statistics.reported % 16) == 1)
      ...
      for (int i = 0; i < MAX_HEADER; i++)
	{
      ...
      // This is the really trick part to get a nicely adjusted header for
...
// increasing intervals.  It can seen as a diversification method with
// respect to the selected phases (since phase saving might be be considered
// to be too stubborn).  However, in combination with reusing best phases
// and in particular target phases has more an intensification flavor.
...
// autarky simplification yet which allows to save partial satisfying
// assignments. Both are implemented in Kissat.
			  ...
			  struct clauses *candidates)
{
  ...
  // learned clauses are kept if they have the same size and glue.  Without
  // radix sort we need to use clause id's as tie-breaker anyhow and thus
  // reversing the redundant clauses on the candidate is not necessary but
  // also not harmful.
...
// Starting and ending of a mode is indicated by a '{' and '}' pair for
// focused mode (the initial mode) and '[' and ']' for stable mode.
...
// interval for the length of the remaining mode phases.  To indicate that
// we moved to a ticks based limits we set 'limits->mode.conflicts' to zero
// after the first mode switch.  Accordingly when testing for switching we
// first check whether that is zero and then use ticks instead of conflicts.
...
set_new_mode_switching_limit (struct satch *solver, uint64_t switched)
{
  ...
  const uint64_t scaled =
    scale_interval (interval, quadratic, switched / 2);
...
void
satch_add_quaternary_clause (struct satch * solver,
